...
```text
sc_main()                          [platforms/basic_cortexM/cosim_main.cpp]
  │
  ├── sc_clock              clk
  ├── PeripheralBusSignals  bus          (shared signal bundle)
  ├── Timer                 dut          ← USER MODEL (Timer/timer.h)
  ├── TlmPinBridge          bridge       (wrapper/tlm_pin_bridge.h)
  ├── TlmAddressMap         interconnect (wrapper/tlm_address_map.h)
  └── CosimServer           server       (wrapper/cosim_server.cpp)
        server.initiator ──────────────► interconnect.cpu_socket
```
...
| `bind_pin_bridge(bridge, bus)` | `wrapper/tlm_pin_bridge.h` | `TlmPinBridge` ↔ same bus signals |
| `server.initiator.bind(...)` | `platforms/basic_cortexM/cosim_main.cpp` | `CosimServer` → `TlmAddressMap` |
...
| platforms/*/cosim_main | Wire YOUR model into wrapper + start server  |
...
| `platforms/basic_cortexM/firmware/` | Baremetal image for Cortex-M3 |
...
* Bare-metal Firmware Development — **implemented** (`platforms/basic_cortexM/firmware`)
