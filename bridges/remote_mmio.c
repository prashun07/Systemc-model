                                ...
                                uint32_t *rdata)
{
    ...
    /*
     * Protocol v2: sample SystemC IRQ levels after every completed access
     * and drive SysBus IRQ lines into the NVIC.
     */
    if (resp.irq_bits != s->irq_last) {
        uint32_t changed = resp.irq_bits ^ s->irq_last;
        int i;

        for (i = 0; i < REMOTE_MMIO_MAX_IRQ; i++) {
            if (changed & (1u << i)) {
                qemu_set_irq(s->irqs[i], (resp.irq_bits >> i) & 1);
            }
        }
        s->irq_last = resp.irq_bits;
    }
    return 0;
}
