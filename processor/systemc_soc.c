 ...
 *   0x40000000  PL window (1 MiB) — remote-mmio -> SystemC virtual SoC
 */
...
#define BRIDGE_SIZE  0x00100000ull /* 1 MiB PL window — Timer/GPIO/UART/SRAM/SysCtrl */
#define HCLK_FRQ     100000000ULL /* match SystemC sys_clk (LT, 10 ns) */
...
static void systemc_soc_init(MachineState *machine)
{
    ...
    /* IRQ levels arrive on each CosimResponse.irq_bits (protocol v2). */
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(bridge), 2, qdev_get_gpio_in(armv7m, 2));
    sysbus_connect_irq(SYS_BUS_DEVICE(bridge), 3, qdev_get_gpio_in(armv7m, 3));
    sysbus_connect_irq(SYS_BUS_DEVICE(bridge), 4, qdev_get_gpio_in(armv7m, 4));
